...
- `illuminate/cache`, `illuminate/http`, `illuminate/support`, and `illuminate/process` cover stable unit-bearing APIs
  across Laravel 11 through 13.
...
Continue the Laravel 11 through 13 integration sweep with `illuminate/queue`, `illuminate/filesystem`, and
`illuminate/cookie`. Preserve version-specific upstream alternatives when a signature differs
