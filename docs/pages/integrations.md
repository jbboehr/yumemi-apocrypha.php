...
Enable `illuminate/mail` to require `second` values when mail is queued for later delivery and when the queued-mail
job's execution timeout is inspected or assigned.

| API concern                                                       | Unit     |
| ----------------------------------------------------------------- | -------- |
| `later()` and `laterOn()` delay                                   | `second` |
| `SendQueuedMailable::$timeout`, preserving its runtime null state | `second` |

Laravel's mail contracts, `Mailable`, `Mailer`, and `PendingMail` all publish the same date-time, interval, or integer
...
function scheduleInvoiceMail(Mailer $mailer, Mailable $invoice): void
{
    $mailer->later(unit(30, 'second'), $invoice);
    //! Mailer::later() expects DateInterval|DateTimeInterface|unit_int<'second'>, 1&unit_int<'minute'> given
    $mailer->later(unit(1, 'minute'), $invoice);
}
```

Transport configuration arrays and `SendQueuedMailable::backoff()` remain unbranded because Laravel does not publish a
single fixed unit-bearing shape for those dynamic values. A timeout property declared by an application mailable keeps
that application's declared type; Apocrypha brands Laravel's `SendQueuedMailable` wrapper property. Laravel 13 queue
attributes belong to the Queue integration, not this Mail surface.
