...
defect. Write for a PHP developer who understands Composer and basic PHPStan usage but does not yet understand Yumemi
Apocrypha's integration model.
...
### Explain the package boundary

Keep the division of responsibility explicit:

- Yumemi defines branded native types, unit semantics, and optional `@yumemi-*` tag promotion.
- Yumemi Apocrypha describes unit-bearing boundaries in third-party packages and selects only verified stubs.
- The upstream package owns runtime behavior. Apocrypha neither wraps calls nor converts values at runtime.

Do not imply that an integration changes Laravel or another dependency at runtime. A branded value remains an ordinary
PHP scalar; Apocrypha supplies additional information to PHPStan only.

### README scope

The root `README.md` is a concise project landing page, not the complete manual. It should normally contain the banner
and title, a short description and status statement, basic installation, one compact integration example, documentation
links, and a concise license summary.

Move complete integration coverage, version policy, configuration alternatives, maintenance procedures, and limitations
into the documentation source. Do not substantially expand the README without a strong reason.
...
- explicitly selected
- autodetected
- verified package major
- upstream signature

Define unfamiliar terms at first use. Prefer plain language before formal language. For example:

> A branded native value is still an ordinary PHP float. Apocrypha tells PHPStan which unit an upstream API expects, but
> runtime PHP does not receive that unit.
...
Examples must be small, realistic, technically correct, focused on one upstream boundary, consistent with the installed
package versions, and statically verifiable through the isolated consumer suite where the repository supports it.
...
Never include an expected diagnostic or inferred type without verifying it against both Yumemi and the real upstream
package. Synthetic declarations alone do not establish integration compatibility.

### Semantic accuracy

Document the exact unit required by each annotated parameter, return, or property. Do not substitute a familiar unit
name when upstream behavior uses a different scale; for example, an API that multiplies a value by 1024 bytes must not
be described as accepting the decimal UDUNITS `kilobyte`.

Distinguish dimensional compatibility from definitional equivalence. Native API boundaries do not perform hidden
conversion, so a dimensionally compatible but differently scaled branded value may still be rejected.

State supported package majors as a verified matrix, not an open-ended promise. Unknown future majors must not receive
stale stubs silently under the default configuration. Document important limitations prominently enough that users can
...
For onboarding or information-architecture changes, also verify that a new user can identify Apocrypha's purpose,
install it, activate an integration, observe a deliberate unit error, and find the exact supported API and version
matrix without first learning the loader implementation.
...
Doctrine applies to authored first-party PHP source under `src/`.
